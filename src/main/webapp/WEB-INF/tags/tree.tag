...
<c:forEach var="issue" items="${items}"	varStatus="count">
	<div class="issue ${fn:toLowerCase(issue.status.name)}" style="margin-left: ${indent}px">
		...
		<div class="name">
			<a href="${pageContext.request.contextPath}/issue/edit/${issue.id}.html">${issue.name}</a>
			<c:if test="${not issue.status.name.equals('Closed')}">
				 &nbsp;&nbsp;&nbsp; <a href="${pageContext.request.contextPath}/issue/create/${issue.project.id}/${issue.type.id}/${issue.id}">Create Sub-Issue</a>
			</c:if>
		</div>
